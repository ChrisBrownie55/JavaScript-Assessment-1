...
const capitalizeI = str =>
  str
    .split(' ')
    .map(
      word =>
        word.replace(/[,\.\!\?]/g, '') === 'i' ? word.toLocaleUpperCase() : word
    )
    .join(' ');
console.log(capitalizeI(sentence));
...
const countA = str => str.match(/[aA]/g).length;
...
const upperCaseScream = str => str.replace(/scream/gi, 'SCREAM');
